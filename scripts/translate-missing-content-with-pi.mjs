...
function parseArgs(argv) {
  const options = {
    ...
    model: process.env.PI_MODEL ?? 'gh/gpt-codex-5.3',
    provider: null,
    verbose: false,
  };
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (argument === '--pi' || argument === '--claude') {
      options.piExecutable = argv[index + 1] ?? options.piExecutable;
      index += 1;
      continue;
    }
...
export function buildPiPrompt(job, sourceContent) {
  return [
    '你是 HagiCode 文档仓库的专业翻译助手，请使用 Pi CLI 完成翻译。',
...
export function sanitizePiOutput(output) {
...
function assertTranslatedOutput(sourceContent, translatedContent, job) {
  if (!translatedContent.trim()) {
    throw new Error(`Pi returned empty output for ${job.targetRelativePath}`);
  }

  if (normalizeComparableText(sourceContent) === normalizeComparableText(translatedContent)) {
    throw new Error(`Pi returned unchanged content for ${job.targetRelativePath}`);
  }
}

async function runPi(prompt, options = {}) {
  const args = buildPiArgs(prompt, options);
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('close', (code) => {
      ...
      reject(new Error(detail || `Pi CLI exited with code ${code}`));
    });
  });
}
...
async function executeJob(job, options) {
  ...
  const prompt = buildPiPrompt(job, sourceContent);
  ...
  const rawOutput = await runPi(prompt, {
    piExecutable: options.piExecutable,
    cwd: docsRoot,
    provider: options.provider,
    model: options.model,
  });
  const translatedContent = sanitizePiOutput(rawOutput);
...
export async function translateMissingContentWithPi(options = {}) {
...
export async function main(argv = process.argv.slice(2)) {
  ...
  return translateMissingContentWithPi(cliOptions);
}
