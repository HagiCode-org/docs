---
title: Hagicode Product Overview
description: "Understand Hagicode through three themes: Smart, Efficient, and Fun."
---

import { LinkCard, CardGrid } from '@astrojs/starlight/components';
import { Image } from 'astro:assets';
import LiveBroadcastCard from '@/components/LiveBroadcastCard.astro';
import ProductVideoShowcase from '@/components/ProductVideoShowcase.astro';
import proposalDrivenIllustration from '../img/product-overview/value-proposition-proposal-driven/illustration.png';
import dualModeIllustration from '../img/product-overview/value-proposition-dual-mode/illustration.png';

In one sentence, **Hagicode** is an AI coding assistant built around real development workflows. It helps teams understand code, organize changes, execute tasks, and keep the reasoning behind the work.

This page does not try to list every module. It stays with three themes: **Smart, Efficient, and Fun**.

## What is Hagicode?

Hagicode is not just a tool for generating a few lines of code. It is a workspace for the broader development loop:

- **Understand codebases** before changing them
- **Turn ideas into structured changes** with proposals, tasks, and validation
- **Keep work traceable** so decisions can be reviewed and reused later
- **Connect to current supported Agent CLIs**: Claude Code, Codex, GitHub Copilot, OpenCode, Hermes, QoderCLI, Kiro, Kimi, Gemini, DeepAgents, and Codebuddy

Hermes, Gemini, and DeepAgents are supported through manual local installation or authentication flows. The remaining CLIs expose one-click installation entry points or guided install flows. For setup details, use the [AI Agent CLI Installation](/en/related-software-installation/ai-agent-cli/) page. `IFlowCli` remains legacy compatibility only and is not part of the current active support range.

![Current completed workspace with the session list, commit notes, and top-level actions](../img/screenshots/shared/classic-view/original.png)

![AI Monitoring popover with Agent CLI version, response time, and health status](../img/screenshots/shared/status-of-agent-clis/original.png)

## Smart: understand first, then act

The “Smart” part of Hagicode is less about flashy demos and more about making AI useful in real projects.

<Image
  src={proposalDrivenIllustration}
  alt="Proposal-driven workflow illustration"
  width={512}
  height={512}
  class="product-overview-illustration"
/>

> Align on goals, scope, and validation before implementation starts.

### 1. Proposal-driven instead of blindly editing

For larger changes, Hagicode can turn an idea into a proposal before implementation starts. That gives you a shared view of:

- the goal
- the scope
- the task breakdown
- the validation criteria

This makes AI feel more like a planning partner than an auto-editor.

![New Idea drawer showing project selection, repository scope, and the proposal request input](../img/screenshots/shared/create-proposal-drawser/original.png)

![Proposal session detail view with workflow steps, execution results, and preserved context](../img/screenshots/shared/classic-proposal-view/original.png)

### 2. Read-only / edit dual mode

Sometimes you want analysis, not changes. Hagicode separates those two cases clearly:

- **Read-only mode** for exploring unfamiliar code, tracing bugs, and understanding architecture
- **Edit mode** for implementing features, fixing defects, and refactoring with intent

That boundary makes it easier to trust the workflow.

### 3. Project context matters

Hagicode focuses on project-level understanding instead of isolated code snippets:

- it looks at repository structure and existing patterns
- it supports multi-turn conversation, task tracking, and tool usage
- it works better as a long-running engineering partner

### 4. Team knowledge stays visible

Proposal and archive flows help preserve the reason behind a change, not just the final diff. That matters when teams grow, hand work off, or revisit earlier decisions.

## Efficient: turn common actions into one smooth flow

The “Efficient” part of Hagicode is about reducing context switching and helping work move in a natural sequence.

<Image
  src={dualModeIllustration}
  alt="Read-only and edit dual-mode illustration"
  width={512}
  height={512}
  class="product-overview-illustration"
/>

> Understand first, then choose whether the AI should move into editing work.

![Current session and project status board screenshot](../img/screenshots/shared/screenshot-a1c2f0f1/original.png)

> The current product already combines session progress, project status, and repository context into one board.

![Session kanban view with Pending, In Progress, and Archived columns](../img/screenshots/shared/kanban-view/original.png)

### 1. One workflow from understanding to delivery

A common workflow looks like this:

1. import or create a project
2. inspect the code in a read-only session
3. break the change down in a proposal session
4. switch to edit mode to implement
5. generate a better commit message with AI

That is simpler than spreading the same work across disconnected tools.

### 2. Monospec for multi-repository work

If your work spans more than one repository, Monospec gives you a unified view:

- coordinate multiple repositories in one place
- keep conventions aligned across repos
- give AI a wider context for cross-repo analysis

It works well for microservices, multi-module products, and long-lived platform work.

### 3. AI Compose Commit for cleaner history

Good code changes still need clear commit history. Hagicode helps generate commit messages that are easier to read and easier to maintain:

- closer to Conventional Commits style
- clearer for reviewers and teammates
- more useful for release notes, rollback, and audits

### 4. Clear setup paths

Whether you prefer Desktop, Docker Compose, or a specific Agent CLI stack, the docs aim to keep the setup path explicit so you can start faster with less guesswork.

### 5. OmniRoute separates model routing from the CLI itself

Hagicode integrates OmniRoute so the CLI you prefer and the model subscription you prefer do not have to be the same decision.

- keep your familiar CLI workflow
- centralize model endpoints and catalog access
- reuse one routing strategy across multiple CLIs

![OmniRoute settings dashboard with enable toggle, endpoint settings, and runtime cards](../img/screenshots/shared/omniroute-support/original.png)

![Agents Workspace model creation screen with model cards and a create-model form](../img/screenshots/shared/omniroute-enabled-model-selector/original.png)

### 6. Vault and code-server keep reference material close to execution

Hagicode also treats knowledge storage and browser-based editing as first-class workflow pieces instead of optional side tools.

- Vault keeps reusable references, notes, and managed directories in one place
- code-server lets you reopen the right workspace quickly when execution moves into editing

![Vault workspace management screen with user and system vault cards](../img/screenshots/shared/vault-view/original.png)

![Code Server runtime summary with recent launches and reopen actions](../img/screenshots/shared/code-server-status/original.png)

### 7. Settings and small utilities stay visible

Long-running tools are easier to keep open when everyday utilities are treated as part of the real product surface, not hidden leftovers.

![General settings screen with sidebar navigation and runtime preference cards](../img/screenshots/shared/settings-view/original.png)

![Hydration reminder overlay with countdown and a Drink Now action](../img/screenshots/shared/hydration-reminder/original.png)

### 8. Skills let the workspace keep growing

Hagicode is not a sealed product. It can keep extending through installable skills, local inventory, and gallery discovery.

![Local Skills inventory with installed paths and batch actions](../img/screenshots/shared/local-skills-view/original.png)

![Skills Gallery with search, source filters, and install counts](../img/screenshots/shared/skills-gallary/original.png)

## Fun: visible progress matters

The “Fun” part of Hagicode is not decoration. It is feedback for long-running engineering work.

### 1. Achievements make progress legible

Actions such as creating sessions, pushing proposals forward, and finishing work can show up as explicit progress rather than disappearing into a scrollback log.

![Achievements dashboard with daily and global progress cards](../img/screenshots/shared/achivement-view/original.png)

### 2. Token use is visible while the session is still live

Instead of waiting for costs or performance issues to appear later, Hagicode surfaces token throughput and recent usage while the session is still active.

![Token throughput panel with live rates, recent calls, and total counters](../img/screenshots/shared/token-metrics/original.png)

## Who is Hagicode for?

| Role | What you get |
| --- | --- |
| New engineers | Faster onboarding into unfamiliar codebases |
| Developers | Less switching between analysis, implementation, and commit cleanup |
| Technical leads | Better control over complex changes and decision traceability |
| Multi-repo teams | A more unified way to coordinate work across repositories |

## Start here

### Quick start

<CardGrid>
  <LinkCard
    title="Wizard Setup"
    href="/en/quick-start/wizard-setup"
    description="The current canonical entry: finish first-project onboarding in the wizard and bind it to a real repository."
  />
  <LinkCard
    title="Create a Conversation Session"
    href="/en/quick-start/conversation-session"
    description="Use the default read-only flow to understand a project first, then switch to edit mode when execution is needed."
  />
  <LinkCard
    title="Create a Proposal Session"
    href="/en/quick-start/proposal-session"
    description="Turn vague ideas into an executable plan for larger or collaborative work."
  />
</CardGrid>

### Installation options

<CardGrid>
  <LinkCard
    title="Desktop"
    href="/en/installation/desktop"
    description="A guided desktop flow for users who want to get started quickly."
  />
  <LinkCard
    title="Docker Compose"
    href="/en/installation/docker-compose"
    description="A good fit for isolated environments, shared setups, or team deployment."
  />
</CardGrid>

### Explore key capabilities

<CardGrid>
  <LinkCard
    title="Monospec"
    href="/en/guides/monospecs"
    description="See how Hagicode approaches multi-repository collaboration."
  />
  <LinkCard
    title="AI Compose Commit"
    href="/en/guides/ai-compose-commit"
    description="Learn how AI can help produce cleaner, more useful commit messages."
  />
  <LinkCard
    title="Skills"
    href="/en/guides/skills"
    description="See local skills, recommendations, and the gallery for expanding the workspace over time."
  />
</CardGrid>

<ProductVideoShowcase locale="en" />

---

For deeper reading, start with quick start, then open the guide for the capability you actually plan to use next.

Live broadcast notices are optional. The default docs build keeps this page quiet; set `VITE_FEATURE_LIVE_BROADCAST_ENABLED=true` when you want to surface the live entry here again.

<LiveBroadcastCard locale="en" />
