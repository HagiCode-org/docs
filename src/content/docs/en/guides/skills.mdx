...
This page explains how to use the `Skills` panel in HagiCode.
...
For your first pass, start with `Local Skills`, then move to `Recommendation` and `Gallery`, and finally review the details panel before installing.
:::
...
`Trusted Providers` controls which sources receive the `trusted` label. Open this page when you want to understand why a source is trusted, or when you need to adjust your trust rules.
...
For a typical first-time workflow:
