...
After Desktop installation and the first launch, Hagicode opens the initialization wizard automatically. The current flow no longer splits first-run work into disconnected entry points. Instead, it keeps the setup path linear: prepare the tools, organize the default executors, create the first project, and then hand off to the next documentation steps.
...
This page is not mainly about manual installation. Its job is to confirm that the environment is ready before the workflow continues to AgentCLI and project setup.
...
You can think of this as choosing the product's default voice: the feature set stays the same, but the wording and presentation feel different.
...
This matters because it keeps the workflow from stalling when one member, backend, or model becomes unavailable. It is especially useful when you enable multiple AgentCLI providers or multiple model options.
...
<CardGrid>
  ...
  <LinkCard
    ...
    description="Click here to see how to start a conversation session after initialization is complete."
  />
</CardGrid>
