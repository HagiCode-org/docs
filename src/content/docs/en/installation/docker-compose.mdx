...
This deployment path starts PostgreSQL through containers, so you usually do not need to install PostgreSQL separately on the host machine. Only prepare a standalone PostgreSQL instance if you intentionally switch to an external database configuration.
:::
...
Now you have successfully deployed Hagicode. Next, connect a real project in the web interface, then continue with [Create Conversation Session](/en/quick-start/conversation-session) or [Create Proposal Session](/en/quick-start/proposal-session).
