---
title: Hagicode Multi-AI Provider Switching and Interoperability Implementation
date: 2026-03-09
tags: [HagiCode, AI Provider, Claude, Codex, Architecture, Interoperability]
description: "In the modern development tool ecosystem, developers frequently rely on various AI coding assistants to support their development workflows. Anthropic's Claude Code CLI and OpenAI's Codex"
---

## Hagicode Multi-AI Provider Switching and Interoperability Implementation

In the modern development tool ecosystem, developers frequently need to use different AI coding assistants to aid their development work. Anthropic's Claude Code CLI and OpenAI's Codex CLI each have their advantages: Claude is renowned for its exceptional code understanding and long-context processing capabilities, while Codex excels in code generation and tool usage.

This article will delve into how the hagicode project implements seamless switching and interoperability between multiple AI providers, including core architecture design, key implementation details, and considerations in practice.

## Background

### Problem Domain

The core challenge facing the hagicode project is supporting multiple AI CLIs within the same platform, enabling users to:

1. Flexibly switch between different AI providers based on requirements
2. Maintain continuity of session state during switching
3. Unify the abstraction of API differences across different CLIs
4. Reserve extension space for adding new AI providers in the future

### Technical Challenges

1. **Unified Interface Differences**: Claude Code CLI is invoked via command line, Codex CLI uses JSON event streams
2. **Streaming Response Handling**: Both providers support streaming responses, but data formats differ
3. **Tool Call Semantics**: Claude and Codex have different representations and lifecycle management for tool calls
4. **Session Lifecycle**: Properly manage session creation, recovery, and termination for each provider

## Analysis

### Architecture Design Philosophy

hagicode adopts the **Provider Pattern** combined with the **Factory Pattern** to abstract AI service calls. The core philosophy of this design is:

1. **Unified Interface Abstraction**: Define the `IAIProvider` interface as a unified abstraction for all AI providers
2. **Factory Creates Instances**: Dynamically create corresponding provider instances via `AIProviderFactory` based on type
3. **Intelligent Selection Logic**: Use `AIProviderSelector` to automatically select the most suitable provider based on scenarios and configuration
4. **Session State Management**: Persist the binding relationship between sessions and CLI threads through database
...
| `AIProviderSelector` | Intelligent provider selection | C# |
| `ClaudeCodeCliProvider` | Claude Code CLI implementation | C# |
| `CodexCliProvider` | Codex CLI implementation | C# |
| `AgentCliManager` | Desktop CLI management | TypeScript |
...
public interface IAIProvider
{
    ...
    /// <summary>
    /// Check provider connectivity and response speed
    /// </summary>
    Task<ProviderTestResult> PingAsync(CancellationToken cancellationToken = default);

    /// <summary>
    /// Send message with embedded command
    /// </summary>
...
Key features of the interface design:
- **Unified Request/Response Model**: All providers use the same `AIRequest` and `AIResponse` types
- **Streaming Support**: Unified streaming output through `IAsyncEnumerable<AIStreamingChunk>`
- **Capability Description**: `ProviderCapabilities` describes the provider's supported features (streaming, tools, max tokens, etc.)
...
This enumeration provides a type-safe representation for all providers supported by the system.

### 3. Factory Pattern Implementation

**AIProviderFactory** is responsible for creating and managing provider instances:
...
- **Dependency Injection**: Create instances through `IServiceProvider`, supporting dependency injection
- **Configuration Driven**: Read provider configuration from configuration files
- **Exception Handling**: Return null on creation failure, facilitating upper-level handling

### 4. Intelligent Selector

**AIProviderSelector** implements provider selection strategy:
...
public class AIProviderSelector : IAIProviderSelector
{
    ...
    public async Task<AIProviderType> SelectProviderAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        // 1. Try to get provider from scenario mapping
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // Use cache to reduce Ping calls
...
Selector strategy:
- **Scenario Mapping Priority**: First check if the business scenario has a specific provider mapping
- **Default Provider Fallback**: Use default provider when scenario mapping fails
- **Fallback Chain Safety**: Try providers in the fallback chain one by one
- **Availability Caching**: Cache provider availability check results to reduce Ping calls
...
Claude Code CLI provider features:
- **Stream Manager Integration**: Use `IClaudeStreamManager` to communicate with Claude CLI
- **CessionId Session Isolation**: Use `CessionId` as the unique session identifier, distinct from system sessionId
- **Working Directory Configuration**: Support configuring working directory, permission mode, etc.
- **Tool Support**: Support tool permission configurations like AllowedTools, DisallowedTools
...
public class CodexCliProvider : IAIProvider
{
    ...
    private CodexThread ResolveThread(Codex codex, AIRequest request)
    {
        ...
        // Check if there's already a bound thread
...
Codex CLI provider features:
- **JSON Event Stream Processing**: Parse Codex's JSON event streams (TurnStarted, ItemStarted, TurnCompleted, etc.)
- **Session Thread Binding**: Persist session-thread binding relationships using SQLite database
- **Thread Reuse**: Support resuming existing threads to maintain session continuity
- **Tool Call Tracking**: Track active tool call states to properly handle tool lifecycle

### 7. Session Thread Binding Mechanism

Codex CLI uses SQLite database to persist session-thread bindings:
...
- **Session Recovery**: Previous sessions can be recovered after system restart
- **Thread Reuse**: The same session can reuse existing Codex threads
- **Auto Cleanup**: Bindings older than 30 days are automatically cleaned up

### 8. Desktop CLI Management
...
export enum AgentCliType {
  ...
  Codex = 'codex',
  // Future extensible: Aider, Cursor, and other CLIs
}
...
ipcMain.handle('llm:call-api', async (event, manifestPath, region) => {
  ...
  try {
    ...
    // Execute using corresponding CLI
...
### 9. Codex Internal Model Provider System

Codex itself also supports multiple model providers, configured through `ModelProviderInfo`:
...
Codex model provider support:
- **Built-in Providers**: OpenAI, Ollama, LM Studio
- **Custom Providers**: Users can add custom providers in config.toml
- **Retry Strategy**: Configurable request and stream retry counts
- **WebSocket Support**: Some providers support WebSocket transmission

## Practice

### Configuration Example

**appsettings.json** configures multiple providers:
    ...
    await foreach (var chunk in provider.StreamAsync(request))
    {
        ...
        switch (chunk.Type)
        {
            case StreamingChunkType.ContentDelta:
                // Display text content in real-time
            ...
            case StreamingChunkType.Metadata:
                // Handle completion events and statistics
                if (chunk.IsComplete)
                {
...
### Usage Example - OpenSpec Command
...
Before switching providers, it's recommended to call `PingAsync` first to ensure the target provider is available:
...
public async Task<bool> IsProviderHealthyAsync(AIProviderType providerType)
{
    ...
    return testResult.Success &&
           testResult.ResponseTimeMs < 5000; // Response within 5 seconds is considered healthy
}
```
...
{
    CessionId = CessionId.New(),  // Generate unique ID
...
The fallback mechanism when a provider is unavailable should be robust, ensuring at least one available provider:
...
Validate all configured provider settings at startup to avoid runtime errors:
...
public void ValidateConfiguration(AIProviderOptions options)
{
    ...
    {
        // Validate executable path (CLI type providers)
        ...
        // Validate API key (API type providers)
...
Provider instances are cached; pay attention to lifecycle management and memory usage:
...
public void ClearInactiveProviders(TimeSpan inactiveThreshold)
{
    ...
    {
        // Assume provider has LastUsedTime property
...
Log provider selection, switching, and execution processes in detail for debugging:
...
Use concurrent collections like `ConcurrentDictionary` to ensure thread safety:
...
Consider data migration when session-thread binding database structure changes:
...
public class SessionThreadMigration
{
    public async Task MigrateAsync(string dbPath)
    {
        ...
        if (version >= 2) return; // Already latest version

        using var connection = new SqliteConnection(dbPath);
        connection.Open();

        // Migrate to v2: Add CreatedAtUtc column
        if (version < 2)
        {
...
Through the combination of provider pattern, factory pattern, and selector pattern, hagicode implements a flexible, extensible multi-AI provider architecture:

- **Unified Interface Abstraction**: `IAIProvider` interface shields differences between different CLIs
- **Dynamic Instance Creation**: `AIProviderFactory` supports runtime provider instance creation
...
The advantages of this architecture design are:
1. **Extensibility**: Adding new AI providers only requires implementing the `IAIProvider` interface
2. **Testability**: Providers can be independently tested and mocked
3. **Maintainability**: Each provider's implementation is independent with single responsibility
4. **User-Friendly**: Supports scenario automatic selection and manual switching

Through this design, hagicode successfully achieves seamless switching and interoperability between Claude Code CLI and Codex CLI, providing developers with a flexible and powerful AI coding assistant experience.
...
- Copyright Notice: All articles in this blog are licensed under BY-NC-SA unless otherwise stated. Please indicate the source when reprinting!
