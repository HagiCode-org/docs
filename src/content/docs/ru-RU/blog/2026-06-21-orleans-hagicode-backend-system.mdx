...
Эти вещи не придуманы на пустом месте. Схема, делящаяся в этой статье, происходит из реального опыта ошибок в проекте [HagiCode](https://www.hagicode.com). HagiCode — это рабочий стол для AI-совместного программирования, его бэкенд должен координировать десятки AI CLI-инструментов в одном процессе, а также предоставлять фронтенду низкозадерженные ответы в реальном времени — проще говоря, и лошадь должна бежать, и не есть траву, и петь пока бежит.
