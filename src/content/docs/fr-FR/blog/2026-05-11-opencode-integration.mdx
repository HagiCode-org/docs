...
description: Retour sur l’intégration d’OpenCode dans HagiCode, du modèle par processus isolés vers un runtime partagé, avec les principaux pièges de session, d’erreur et de ressources.
---
...
La solution partagée dans cet article provient de notre expérience pratique dans le projet [HagiCode](https://www.hagicode.com). HagiCode est un projet d'assistant de code basé sur l'IA. Au cours du développement, nous devions intégrer plusieurs AI Providers, et OpenCode est l'un d'entre eux. Le processus d'évolution architecturale partagé ci-dessous est une expérience réelle obtenue en trébuchant et en optimisant dans notre projet actuel — après tout, nous n'avions pas le choix, les pièges rencontrés devaient être comblés.
...
- Client de bureau HagiCode Desktop : [hagicode.com/desktop/](https://www.hagicode.com/desktop/)
