...
Premièrement, la charge de maintenance est énorme. 28 langues multipliées par 2 champs équivalent à 56 blocs de contenu à gérer. Modifier manuellement en changeant de langue dans le backend du site Steamworks n'est pas très efficace. Chaque mise à jour de contenu nécessite de répéter ce processus, c'est une corvée qu'on ne souhaite plus revivre.
...
Les plus courantes sont `en-US` (anglais), `zh-CN` (chinois simplifié), `zh-TW` (chinois traditionnel), `ja-JP` (japonais) et `ko-KR` (coréen). Après tout, ces langues couvrent les principaux marchés, une fois celles-ci maîtrisées, les autres semblent moins effrayantes.
...
```typescript
// 28 codes de langue pris en charge
