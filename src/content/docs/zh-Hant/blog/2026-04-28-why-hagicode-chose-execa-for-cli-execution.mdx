...
本文分享的方案來自我們在 [HagiCode](https://www.hagicode.com) 專案中的實踐經驗。HagiCode 是一個 AI 程式碼助手專案，需要在多個子專案（Hagiscript 腳本引擎和 Desktop 桌面應用）中執行大量外部命令。這種多語言、多平台的複雜度，或許就是我們引入 execa 的直接原因。
