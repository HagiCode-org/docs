...
HagiCode 文件部落格發布了一種全語言提要以及與桌面語言選擇器一致的語言提要。您可以繼續使用現有的全語言提要或僅訂閱特定語言的貼文。

| 適用範圍 | RSS 網址 |
|-------|---------|
| 所有語言 | `https://docs.hagicode.com/blog/rss.xml` |
...
`https://docs.hagicode.com/blog/rss.xml` 是現有的相容性來源，並繼續包含每種支援語言的貼文。現有的提要閱讀器、自動化腳本和 README 同步工作流程不需要變更。英文提要保持不變 `rss.en.xml` 並且還暴露了 `rss.en-US.xml` 桌面規範代碼。
...
- 如果貼文未設定顯式語言元數據，則語言將從內容路徑派生： `src/content/docs/blog/` 是 `zh-CN`, `src/content/docs/en/blog/` 是 `en-US`，其他語言樹使用它們的目錄名稱。
