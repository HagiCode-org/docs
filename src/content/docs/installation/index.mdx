---
title: 安装指南
description: 选择适合你的 HagiCode 安装方式，并继续完成相关软件准备。
sidebar:
  order: 1
---

import { CardGrid, LinkCard } from '@astrojs/starlight/components';

HagiCode 提供多种安装方式。先选适合自己的部署路径，再按需补充相关软件即可。若你正在使用 Windows，并且希望走当前主推的桌面分发渠道，请优先阅读 Microsoft Store 安装。

当前与安装准备最相关的 Agent CLI 活跃支持范围为：Claude Code、Codex、GitHub Copilot、OpenCode、Hermes、QoderCLI、Kiro、Kimi、Gemini、DeepAgents、Codebuddy。其中 Hermes、Gemini、DeepAgents 目前采用手动安装或认证流程，其余 CLI 提供一键安装入口或引导安装流程。

## 选择安装方式

<CardGrid>
  <LinkCard
    title="Microsoft Store 安装"
    href="/installation/windows-store"
    description="当前 Windows 默认推荐入口，适合直接通过官方商店完成安装与后续更新。"
  />
  <LinkCard
    title="Desktop 安装"
    href="/installation/desktop"
    description="图形化安装与管理流程。Windows 用户若不走商店渠道，或 macOS / Linux 用户，可从这里进入手动安装说明。"
  />
  <LinkCard
    title="Docker Compose 安装"
    href="/installation/docker-compose"
    description="适合需要环境隔离、统一部署或团队协作的场景。"
  />
</CardGrid>

## 相关软件安装

如果你的工作流需要额外工具或运行时，可以继续阅读这些配套指南：

<CardGrid>
  <LinkCard
    title="安装 Node.js"
    href="/related-software-installation/nodejs/installation"
    description="为 OpenSpec 以及多数支持一键安装的 Agent CLI 准备 Node.js 与 npm 环境。"
  />
  <LinkCard
    title="安装 OpenSpec"
    href="/related-software-installation/openspec/setup-openspec"
    description="安装规范驱动开发工具，管理提案、规格与变更。"
  />
  <LinkCard
    title="安装 AI Agent CLI"
    href="/related-software-installation/ai-agent-cli/"
    description="统一查看 11 个当前活跃支持的 Agent CLI 官方安装入口，并区分一键安装与手动安装/认证。"
  />
</CardGrid>

## 下一步建议

- Windows 用户可先阅读 [Microsoft Store 安装](/installation/windows-store)。
- 若你需要手动下载安装包，或正在使用 macOS / Linux，可阅读 [Desktop 安装](/installation/desktop)。
- 若你在团队或服务器环境中部署，可阅读 [Docker Compose 安装](/installation/docker-compose)。
- 如果你需要确认某个 Agent CLI 的支持状态、安装方式或历史兼容说明，请直接打开 [AI Agent CLI 安装](/related-software-installation/ai-agent-cli/)。
