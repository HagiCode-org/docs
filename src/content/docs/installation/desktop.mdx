...
:::caution[Mac 版本状态]
当前暂不提供 Mac 原生安装包，建议安装 Docker 版本：\
[前往 Container 页面](https://hagicode.com/container/)
:::
...
- **Windows**：下载 `.exe` 安装程序
- **macOS**：建议安装 Docker 版本（[前往 Container 页面](https://hagicode.com/container/)）
...
:::tip[默认安装前置条件]
对于最新版本 HagiCode 的默认本地安装流程，通常不需要您先手动安装 PostgreSQL。只有在您准备使用外部数据库、执行高级部署，或按特定运维方案接入独立数据库时，才需要额外参考 PostgreSQL 安装指南。
:::
...
![选择区域](../img/screenshots/installation/install-desktop/03-select-installation-region/original.png)

**区域选择说明**：
- **中国大陆**：优化的网络连接，适合国内用户
- **国际区域**：适合海外用户或有特殊网络需求的用户

![AI 安装向导](../img/screenshots/installation/install-desktop/04-ai-installation-wizard/original.png)

向导会根据您选择的区域自动配置并启动 AI 安装过程。

![AI 安装进行中](../img/screenshots/installation/install-desktop/05-ai-installation-in-progress/original.png)
...
![AI 安装完成](../img/screenshots/installation/install-desktop/06-ai-installation-result/original.png)

![确认安装成功](../img/screenshots/installation/install-desktop/07-confirm-installation-success/original.png)
...
![创建项目向导](../img/screenshots/installation/install-desktop/10-create-project-wizard/original.png)
...
![导入 Git 仓库](../img/screenshots/installation/install-desktop/11-import-git-repository/original.png)
