...
:::tip[推荐的 API Provider]
Hagicode 内置了多个推荐的 API Provider 预设，方便您快速配置：

- **GLM（智谱 AI）**：国内领先的大语言模型服务，提供稳定的 API 接口
- **MiniMax**：专注于对话和生成式 AI 的平台，支持多种模型
- **阿里云 DashScope**：阿里云提供的大语言模型服务，集成方便

选择这些预设可以自动填充正确的 Provider 配置，您只需填写对应的 API Key 即可。
:::

:::note[获取 API Key]
您需要从所选 Provider 官网获取 API Key：
- **GLM**：访问 [智谱 AI 开放平台（推广链接）](https://www.bigmodel.cn/claude-code?ic=14BY54APZA)
- **MiniMax**：访问 [MiniMax 开放平台（推广链接）](https://platform.minimaxi.com/subscribe/coding-plan?code=8wNck4etDM&source=link)
- **阿里云**：访问 [阿里云 DashScope（推广链接）](https://www.aliyun.com/benefit/ai/aistar?userCode=vmx5szbq&clubBiz=subTask..12384055..10263..)
:::
