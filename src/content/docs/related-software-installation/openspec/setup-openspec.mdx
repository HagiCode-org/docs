...
OpenSpec 用来管理提案、规范和变更。本页只做一件事：把安装流程说明清楚，让你能尽快把它接到现有工作流里。

## 什么是 OpenSpec？

OpenSpec 是一个命令行工具，用来管理软件开发过程中的提案（Proposals）与规范（Specifications）。它提供的流程主要包括：
...
3. 参考 [HagiCode 文档站点](/product-overview) 中的相关指南继续排查
