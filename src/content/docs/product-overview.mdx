...
如果你已经理解了 HagiCode 的产品定位，接下来最重要的问题通常就是：我现在应该从哪个版本开始，以及当前公开可用的 Windows 渠道到底是哪一个。

下表中，`✅` 表示支持，`❌` 表示不支持。

| 项目 | Desktop | Container | Microsoft Store |
| --- | --- | --- | --- |
| 获取入口 | [Desktop 安装](/installation/desktop) | [Container 部署](/installation/docker-compose) | [Microsoft Store 安装](/installation/windows-store) |
| 定价 | 免费 | 免费 | 免费 |
| 全部免费特性 | ✅ | ✅ | ✅ |
| Vault | ✅ | ✅ | ✅ |
| Skills | ✅ | ✅ | ✅ |
...
**Windows 渠道状态。** Steam 主应用条目已经退役，当前公开可用的 Windows 渠道以 Microsoft Store 为主。相关背景可见 [为什么 HagiCode 没有上架 Steam](/faq/steam-distribution-status)。

**能力对齐说明。** 当前 Microsoft Store 版本在核心工作流能力上与免费 Desktop 基线保持一致，包括提案、Vault、Skills、OmniRoute、GitHub 集成和本地成就。

**安装方式建议。** 如果你希望走系统商店托管的安装与更新路径，优先使用 Microsoft Store；如果你需要手动控制安装包来源，或当前环境不适合走商店渠道，请改用 [Desktop 安装](/installation/desktop)。
...
<CardGrid>
  <LinkCard
    ...
    description="面向当前公开可用的 Windows 渠道，适合希望直接通过系统商店完成安装与更新的用户。"
  />
  <LinkCard
    title="Steam 分发状态"
    href="/faq/steam-distribution-status"
    description="说明为什么 Steam 主应用条目已经退役，以及当前 Windows 分发渠道为什么改为 Microsoft Store。"
  />
</CardGrid>
