...
本页帮助你了解 HagiCode 中的 `Skills` 面板应该怎么用。

如果你想完成下面这些事情，可以直接打开 Skills：
...
:::tip[建议的使用顺序]
第一次使用时，建议先看本地技能（`Local Skills`），再看推荐（`Recommendation`）和技能库（`Gallery`），最后在技能详情页中确认安装信息。
:::
...
可信来源页（`Trusted Providers`）用来管理“哪些来源会被标记为 trusted”。如果你想知道某个技能为什么显示 trusted，或者你想补充自己的来源规则，就看这个页面。
