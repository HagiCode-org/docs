...
Monospecs 是 HagiCode 提供的多仓库管理方案，通过 `.hagicode/monospecs.yaml` 追踪和管理多个子仓库。凡是需要统一管理多个独立仓库的项目，都可以用它。

> **迁移提醒**：`monospecs.yaml` 已从项目根目录迁移到 `.hagicode/monospecs.yaml`。根目录的 `monospecs.yaml` 不再被检测或兼容回退；已有项目需要手动创建 `.hagicode/` 目录并移动文件。

> 💡 **快速上手**：参考 [monospecs-demo](https://github.com/HagiCode-org/monospecs-demo) 仓库了解完整的 monospecs 项目结构，包含配置文件、拉取脚本和 README 文档。

### 核心概念

**配置文件**

`.hagicode/monospecs.yaml` 是整个管理方案的核心，该文件定义了：
...
|------|---------------|---------------------|
| **仓库发现** | 手动查找和记录 | 从 `.hagicode/monospecs.yaml` 自动读取 |
...
- 存储 `.hagicode/monospecs.yaml` 配置文件
- 管理跨仓库的配置和脚本
- 追踪子仓库的元数据关系
...
在主仓库根目录创建 `.hagicode/monospecs.yaml` 配置文件，添加你需要管理的子仓库：
...
这将根据 `.hagicode/monospecs.yaml` 中的配置克隆所有子仓库到 `repos/` 目录。
...
在 `.hagicode/monospecs.yaml` 的 `repositories` 数组中添加新条目：
...
1. 编辑 `.hagicode/monospecs.yaml` 更新对应条目
2. 验证 YAML 语法正确性
3. 如需同步变更，手动更新本地仓库配置
...
![新建提案时选择子仓库](../img/screenshots/monospecs/create-proposal-select-sub-repositories/original.png)

在创建变更时，OpenSpec 会读取 `.hagicode/monospecs.yaml` 配置，列出所有可用的子仓库供你选择。你可以选择需要修改的子仓库，系统会自动追踪变更涉及的范围。

### 仓库详情预览

将鼠标悬停在项目上时，会显示各个子仓库的详细信息和状态。

![悬停查看仓库详情](../img/screenshots/monospecs/hover-project-show-repository-status/original.png)
...
![AI 提交时选择子仓库](../img/screenshots/monospecs/ai-compose-commit-select-sub-repository/original.png)

AI 提交功能会：
1. 读取 `.hagicode/monospecs.yaml` 配置
2. 分析你的变更内容
3. 建议你应该提交到哪些子仓库
...
**monospecs 管理的项目**：
- OpenSpec 读取 `.hagicode/monospecs.yaml` 识别仓库范围
- 根据变更文件的路径自动匹配目标仓库
...
1. OpenSpec 读取 `.hagicode/monospecs.yaml`
...
1. **同步配置与实际状态**：定期确保 `.hagicode/monospecs.yaml` 反映真实的仓库结构

## 从根目录 monospecs.yaml 迁移

如果项目仍然使用根目录 `monospecs.yaml`，请手动迁移到新的 HagiCode 项目数据目录：

```bash
mkdir -p .hagicode
mv monospecs.yaml .hagicode/monospecs.yaml
git add .hagicode/monospecs.yaml
git rm monospecs.yaml
git commit -m "Move MonoSpecs config to .hagicode"
```

迁移后，HagiCode 只会读取 `.hagicode/monospecs.yaml`。如果只保留根目录 `monospecs.yaml`，项目会被视为未配置 MonoSpecs，相关脚本也不会把它当作兼容路径读取。
