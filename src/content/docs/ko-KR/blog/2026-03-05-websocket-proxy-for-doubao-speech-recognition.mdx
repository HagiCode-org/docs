---
title: 브라우저 WebSocket 인증 문제 해결: 더빠오 음성 인식의 프록시 솔루션 실천
date: 2026-03-05
tags: [WebSocket, Proxy, Speech Recognition, Authentication, Backend]
description: "브라우저 WebSocket API는 사용자 정의 HTTP 헤더를 지원하지 않아 헤더를 통해 인증 정보를 전달해야 하는 음성 인식 서비스에 도전이 됩니다. 이 글은 HagiCode"
---

## 브라우저 WebSocket 인증 문제 해결: 더빠오 음성 인식의 프록시 솔루션 실천

> 브라우저 WebSocket API는 사용자 정의 HTTP 헤더를 지원하지 않아 헤더를 통해 인증 정보를 전달해야 하는 음성 인식 서비스에 도전이 됩니다. 이 글은 HagiCode 프로젝트에서 백엔드 프록시 솔루션으로 이 문제를 해결하는 방법과 playground에서 프로덕션 환경까지의 실천 과정을 공유합니다.


## 배경

사실 HagiCode 프로젝트의 음성 인식 기능을 만들 때, 우리도 자신감满满하게 바이트댄스의 더빠오 음성 인식 서비스를 선택했습니다. 처음 설계는 아주 간단했죠—프론트엔드에서 더빠오의 WebSocket 서비스에 직접 연결하는 것이었습니다. 이게 뭐가 어렵나요? 그냥 연결을 만들고 데이터를 전송하는 것인데요?

그런데 완전 예상치 못하게—더빠오의 API는 HTTP 헤더를 통해 인증 정보를 전달하도록 요구했습니다, accessToken, secretKey 같은 것들 말이죠. 이제 좀 난감해졌습니다, 왜냐하면 브라우저의 WebSocket API는 아예 사용자 정의 헤더 설정을 지원하지 않거든요.

지원하지 않으면 어떡하죠?

그때도 한참 고민했었습니다. 결국 눈앞에 있는 두 가지 선택:

1. 인증 정보를 URL 쿼리 파라미터에 넣기—간단하고粗暴한 방법
2. 백엔드에 프록시 계층 만들기—조금 더 복잡해 보이는 방법

첫 번째方案吧,凭证就直接暴露在前端代码和本地存储里了。这安全吗？反正我是不太敢苟同的。而且有些 API 必须用 header 验证，根本走不通。
