...
この記事で共有するソリューションは、[HagiCode](https://www.hagicode.com) プロジェクトでの実践経験から来ています。HagiCode は AI コーディングアシスタントプロジェクトで、そのデスクトップ製品に code-server を組み込みランタイムとして統合しているため、マルチプラットフォームビルドとリリースのエンジニアリング問題を解決する必要がありました。この件は、突き詰めれば製品を作るためだけのもので、それだけです。
