...
- Chamar a API da Windows Store para implementar compras dentro do aplicativo
- Lidar com a virtualização de sistema de arquivos específica de aplicativos da Windows Store
...
O HagiCode Desktop adota uma solução híbrida: usa dynwinrt para acessar a API da Windows Store, usa extensões nativas Node.js para processar operações de compra de Store de alta performance, e usa os módulos fs e path nativos do Node.js para lidar com a virtualização de sistema de arquivos específica de aplicativos da Windows Store. Simplificar quando possível é também o nosso princípio.

## Solução 1: Usar dynwinrt para chamar WinRT API

dynwinrt é uma cadeia de ferramentas fornecida pela Microsoft que pode gerar automaticamente bindings JavaScript com base em arquivos de metadados do Windows SDK. É especializado em chamar WinRT API, como a Windows Store API.
...
## Solução 3: Tratar virtualização de aplicativos da Windows Store

Aplicativos da Windows Store rodam em um ambiente virtualizado, o mapeamento de caminhos precisa de tratamento especial. O HagiCode Desktop usa a seguinte função para tratar este problema:
...
A maioria da API do Windows Store é assíncrona, use Promise ou async/await. Ao escrever código assíncrono, lembre-se de tratar casos de borda, como timeout, cancelamento e afins. Afinal, o sabor de esperar, ninguém quer provar muito.
