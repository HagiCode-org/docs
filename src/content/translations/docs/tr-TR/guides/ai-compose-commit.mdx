---
title: AI Oluşturma Taahhüt Kılavuzu
description: Git taahhüt mesajlarını akıllıca oluşturmak, taahhüt kalitesini ve tutarlılığını artırmak için yapay zekayı kullanın.
---

## Genel Bakış

AI Compose Commit, mevcut kod değişikliklerini okur ve Geleneksel Taahhütler formatında taahhüt mesajları oluşturur. Mesele basit: geçmişi daha net tutarken taahhüt yazmaya daha az zaman harcayın.

### AI Oluşturma Taahhüdü nedir?

AI Compose Commit, değişikliklerinizi analiz eder ve bunlar için standartlaştırılmış taahhüt mesajları yazar. Daha tutarlı bir geçmiş için manuel taahhüt yazma işlemi yapar.

Şu durumlarda kullanın:
- Birden fazla taahhüt gerektiren büyük kod değişiklikleri
- Koordineli değişikliklerle çoklu veri havuzu projeleri
- Standartlaştırılmış taahhüt formatlarına ihtiyaç duyan ekipler
- Taahhüt kalitesini artırmak isteyen geliştiriciler

### Ana Avantajlar

| Özellik | Geleneksel Manuel Taahhütler | AI Oluşturma Taahhüdü |
|----------|----------------------|------------------|
| **Kaliteye Taahhüt** | Bireysel deneyime bağlıdır | En iyi uygulamalara dayalı, tutarlı kalite |
| **Taahhüt Hızı** | Düşünce ve organizasyon gerektirir | Tek tıklamayla anında oluşturma |
| **Tutarlılık** | Taahhütler arasında değişiklik gösterebilir | Standartları otomatik olarak takip eder |
| **Çoklu Repo Desteği** | Manuel koordinasyon gerekli | Otomatik repo tanıma ve gruplama |
| **Zamandan Tasarruf** | Önemli zaman alabilir | Taahhüt başına 10-60+ saniye tasarruf sağlar |

## Hızlı Başlangıç

### Özelliği Etkinleştir

HagiCode masaüstü uygulamasında AI Compose Commit'i kullanmak istediğiniz projeye gidin ve bu özelliği etkinleştirin.

**Önemli**: Öncelikle projenizin HagiCode'a eklendiğinden emin olun.

### Tetikleme Özelliği

Git işlemleri bölümünde "AI Birleşik Taahhüt" düğmesini bulun.

![AI Birleşik Onaylama Düğmesi](../../img/screenshots/ai-compose-commit/trigger-ai-compose-commit-button/original.png)

Bu düğme genellikle Git işlemleri bölümünde diğer sürüm kontrol eylemlerinin yanında belirgin bir şekilde bulunur.

**Önemli Not**: Bu düğmeye tıklamak otomatik taahhüt mesajı oluşturulmasını tetikleyecektir. Taahhütleri geri almanız veya değiştirmeniz gerekiyorsa standart Git komutlarını kullanın.

### İlk Kullanım Riski İstemi

AI Compose Commit'i ilk kez kullanırken veya onaylanmamış değişikliklerle birlikte sistem bir onay iletişim kutusu görüntüler.

![İkinci Onay İletişim Kutusu](../../img/screenshots/ai-compose-commit/second-confirmation-dialog/original.png)

İletişim kutusu aşağıdakileri içeren seçenekleri içerir:
- Onaylayın ve işleme başlayın
- Riskleri anlayın, artık bu istemi gösterme
- Bu seçeneğin işaretlenmesi sonraki kullanımı daha kolay hale getirir

Otomatik taahhütlerin sonuçlarını anlamak için ilk kullanım sırasında risk istemini dikkatlice okumanızı öneririz.

### Sonuçları Görüntüle

AI işlemeyi tamamladıktan sonra bir başarı bildirimi göreceksiniz.

Commit mesajları oluşturuldu ve hazır. Gerekirse tamamlamadan önce bunları inceleyebilirsiniz.

## Özellik Ayrıntıları

### Yapay Zeka Analiz Mantığı

AI Compose Commit, kod değişikliklerinizi aşağıdaki süreç aracılığıyla analiz eder:

#### 1. Çalışma Alanı Değişikliklerini Okuyun

Sistem, mevcut çalışma alanındaki tüm değiştirilmiş, yeni eklenen ve silinmiş dosyaları toplar.

#### 2. Dosya İçeriğini Analiz Edin

Yapay zeka, aşağıdakileri anlamak için değiştirilen dosyaların içeriğini derinlemesine analiz eder:
- Uygulanan işlevsel değişiklikler
- Hata düzeltmeleri uygulandı
- Kullanılan yeniden düzenleme modelleri
- Bağımlılıklar eklendi veya kaldırıldı

#### 3. Değişim İlişkilerini Belirleyin

Yapay zeka, hangi değişikliklerin birlikte gruplandırılması gerektiğini akıllıca belirler:
- Aynı özellik için ön uç ve arka uç değişiklikleri
- İlgili yapılandırma dosyası değişiklikleri
- Güncellemeleri uygulama değişiklikleriyle birlikte test edin

#### 4. Taahhüt Yapısı Oluşturun

Yapay zeka, Geleneksel Taahhüt spesifikasyonlarına uygun olarak yapılandırılmış taahhüt bilgileri üretir:
...
```
feat(auth): kullanıcı oturum açma işlevini ekle
Google ve GitHub sağlayıcılarıyla OAuth2 kimlik doğrulaması uygulandı.

Oturum açma bileşeni artık şunları destekliyor:
- Google OAuth entegrasyonu
- GitHub OAuth entegrasyonu
- JWT belirteçleriyle oturum yönetimi

Ortak Yazar: Claude Opus 4.6 <noreply@anthropic.com>
```
...
```text
Ortak Yazar: HagiCode <noreply@hagicode.com>
```

Repository override example:

```bash
git config --local coAuthoredBy.name "Platform Botu"
git yapılandırma --local coAuthoredBy.email "platform-bot@example.com"
```
...
git yapılandırma --global coAuthoredBy.email "noreply@hagicode.com"
```

With the local override above, the trailer becomes:

```text
Ortak Yazar: Platform Botu <platform-bot@example.com>
```
