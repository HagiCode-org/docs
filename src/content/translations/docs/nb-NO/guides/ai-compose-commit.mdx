...
description: Bruk AI til å generere Git-commit-meldinger på en intelligent måte, og forbedre commit-kvaliteten og konsistensen.
---

## Oversikt

AI Compose Commit leser gjeldende kodeendringer og genererer commit-meldinger i konvensjonelt commit-format. Poenget er enkelt: Bruk mindre tid på å skrive forpliktelser mens du holder historien klarere.

### Hva er AI Compose Commit?

AI Compose Commit analyserer endringene dine og skriver standardiserte commit-meldinger for dem. Den bytter manuell forpliktelse for en mer konsistent historie.

Bruk den når:
- Store kodeendringer som krever flere forpliktelser
- Flerlagsprosjekter med koordinerte endringer
- Lag som krever standardiserte forpliktelsesformater
- Utviklere som ønsker å forbedre forpliktelseskvaliteten

### Hovedfordeler

| Funksjon | Tradisjonelle manuelle forpliktelser | AI Compose Commit |
|----------|----------------------|------------------|
| **Forpliktelsekvalitet** | Avhenger av individuell erfaring | Basert på beste praksis, konsekvent kvalitet |
| **Forpliktelseshastighet** | Krever tanke og organisering | Ett klikk, øyeblikkelig generering |
| **Konsistens** | Kan variere mellom forpliktelser | Følger automatisk standarder |
| **Multi-repo-støtte** | Manuell koordinering kreves | Automatisk repogjenkjenning og gruppering |
| **Tidsbesparelser** | Kan ta betydelig tid | Sparer 10-60+ sekunder per forpliktelse |

## Rask start

### Aktiver funksjon

I HagiCode-skrivebordsapplikasjonen, naviger til prosjektet der du vil bruke AI Compose Commit og aktiver denne funksjonen.

**Viktig**: Sørg for at prosjektet ditt er lagt til HagiCode først.

### Utløserfunksjon

Finn "AI Combined Commit"-knappen i Git-operasjonsdelen.

![AI Combined Commit-knapp](../../img/screenshots/ai-compose-commit/trigger-ai-compose-commit-button/original.png)

Denne knappen er vanligvis plassert fremtredende i Git-operasjonsdelen, i nærheten av andre versjonskontrollhandlinger.

**Viktig merknad**: Ved å klikke på denne knappen utløses automatisk commit-meldingsgenerering. Hvis du trenger å rulle tilbake eller endre forpliktelser, bruk standard Git-kommandoer.

### Forespørsel om risiko for første bruk

Når du bruker AI Compose Commit for første gang eller med ikke-godkjente endringer, vil systemet vise en bekreftelsesdialog.

![Andre bekreftelsesdialog](../../img/screenshots/ai-compose-commit/second-confirmation-dialog/original.png)

Dialogboksen inneholder alternativer inkludert:
- Bekreft og start behandlingen
- Forstå risikoer, ikke lenger vis denne forespørselen
- Ved å krysse av for dette alternativet blir etterfølgende bruk mer praktisk

Anbefaler å lese risikomeldingen nøye ved første gangs bruk for å forstå implikasjonene av automatiske forpliktelser.

### Se resultater

Etter at AI har fullført behandlingen, vil du se et suksessvarsel.

Commit-meldinger er generert og er klare. Du kan se gjennom dem før du avslutter om nødvendig.

## Funksjonsdetaljer

### AI-analyselogikk

AI Compose Commit analyserer kodeendringene dine gjennom følgende prosess:

#### 1. Les arbeidsområdeendringer

Systemet samler alle endrede, nylig lagt til og slettede filer i gjeldende arbeidsområde.

#### 2. Analyser filinnhold

AI analyserer innholdet i endrede filer dypt for å forstå:
- Funksjonsendringer implementert
- Feilrettinger tatt i bruk
- Refaktoreringsmønstre brukt
- Avhengigheter lagt til eller fjernet

#### 3. Identifiser endringsassosiasjoner

AI identifiserer intelligent hvilke endringer som bør grupperes sammen:
- Frontend og backend endringer for samme funksjon
- Relaterte konfigurasjonsfilendringer
- Testoppdateringer sammen med implementeringsendringer

#### 4. Generer forpliktelsesstruktur

AI genererer strukturert forpliktelsesinformasjon etter konvensjonelle forpliktelser-spesifikasjoner:
...
```
feat(auth): legg til funksjonalitet for brukerpålogging
Implementert OAuth2-autentisering med Google- og GitHub-leverandører.

Påloggingskomponenten støtter nå:
- Google OAuth-integrasjon
- GitHub OAuth-integrasjon
- Sesjonsadministrasjon med JWT-tokens

Medforfatter: Claude Opus 4.6 <noreply@anthropic.com>
```
...
```text
Medforfatter av: HagiCode <noreply@hagicode.com>
```
...
```text
Medforfatter: Platform Bot <platform-bot@example.com>
```
