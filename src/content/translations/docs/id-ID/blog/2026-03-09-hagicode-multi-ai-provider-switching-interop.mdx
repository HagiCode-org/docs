---
title: Rencana Implementasi Peralihan dan Interoperabilitas Penyedia Multi-AI HagiCode
date: 2026-03-09
tags: [HagiCode, AI Provider, Claude, Codex, Architecture, Interoperability]
description: "Rencana Implementasi Peralihan dan Interoperabilitas Penyedia Multi-AI HagiCode: Dalam ekosistem peralatan pengembang modern, pengembang sering kali perlu menggunakan teknologi yang berbeda"
---

## Rencana Implementasi Peralihan dan Interoperabilitas Penyedia Multi-AI HagiCode

Dalam ekosistem alat pengembang modern, pengembang sering kali perlu menggunakan asisten pengkodean AI yang berbeda untuk mendukung pekerjaan mereka. Claude Code CLI dari Anthropic dan Codex CLI OpenAI masing-masing memiliki kekuatannya masing-masing: Claude dikenal karena pemahaman kode yang luar biasa dan penanganan konteks yang panjang, sementara Codex unggul dalam pembuatan kode dan penggunaan alat.

Artikel ini membahas secara mendalam bagaimana proyek HagiCode mencapai peralihan dan interoperabilitas yang lancar di berbagai penyedia AI, termasuk desain arsitektur inti, detail implementasi utama, dan pertimbangan praktis.

## Latar Belakang

### Domain Masalah

Tantangan inti yang dihadapi oleh proyek HagiCode adalah mendukung beberapa AI CLI pada platform yang sama, sehingga pengguna dapat:

1. Beralih secara fleksibel antar penyedia AI berdasarkan kebutuhan mereka
2. Pertahankan kesinambungan sesi selama peralihan penyedia
3. Satukan perbedaan API di berbagai CLI di balik abstraksi umum
4. Cadangan poin ekstensi untuk menambah penyedia AI baru di masa mendatang

### Tantangan Teknis

1. **Menyatukan perbedaan antarmuka**: Claude Code CLI dipanggil melalui panggilan baris perintah, sedangkan Codex CLI menggunakan aliran peristiwa JSON
2. **Menangani respons streaming**: Kedua penyedia mendukung respons streaming, namun dengan format data yang berbeda
3. **Semantik pemanggilan alat**: Claude dan Codex berbeda dalam cara mereka merepresentasikan pemanggilan alat dan mengelola siklus hidupnya
4. **Siklus hidup sesi**: Sistem harus mengelola pembuatan, pemulihan, dan penghentian sesi dengan benar untuk setiap penyedia

## Analisis

### Pendekatan Desain Arsitektur

HagiCode menggunakan **Pola Penyedia** yang dikombinasikan dengan **Pola Pabrik** untuk mengabstraksi pemanggilan layanan AI. Ide inti dari desain ini adalah:

1. **Abstraksi antarmuka terpadu**: Tentukan `IAIProvider` antarmuka sebagai abstraksi umum untuk semua penyedia AI
2. **Instans buatan pabrik**: Gunakan `AIProviderFactory` untuk secara dinamis membuat instance penyedia yang sesuai berdasarkan jenisnya
3. **Logika pemilihan cerdas**: Gunakan `AIProviderSelector` untuk secara otomatis memilih penyedia yang paling sesuai berdasarkan skenario dan konfigurasi
4. **Manajemen status sesi**: Mempertahankan hubungan yang mengikat antara sesi dan thread CLI dalam database

### Komponen Utama

| Komponen | Tanggung jawab | Bahasa |
|------|--------|------|
| `IAIProvider` | Antarmuka penyedia terpadu | C# |
| `AIProviderFactory` | Membuat dan mengelola instans penyedia | C# |
| `AIProviderSelector` | Pilih penyedia dengan cerdas | C# |
| `ClaudeCodeCliProvider` | Implementasi CLI Kode Claude | C# |
| `CodexCliProvider` | Implementasi Codex CLI | C# |
| `AgentCliManager` | Manajemen CLI sisi desktop | skrip ketikan |

## Solusi

### 1. Desain Antarmuka Inti

**`IAIProvider` interface** mendefinisikan abstraksi penyedia terpadu:
...
Karakteristik utama dari desain antarmuka ini:
- **Model permintaan/respons terpadu**: Semua penyedia menggunakan hal yang sama `AIRequest` dan `AIResponse` jenis
- **Dukungan streaming**: Standarisasi keluaran streaming `IAsyncEnumerable<AIStreamingChunk>`
- **Deskripsi kemampuan**: `ProviderCapabilities` menjelaskan fitur-fitur yang didukung oleh penyedia (streaming, alat, token maksimum, dan sebagainya)
- **Perintah tersemat**: `SendMessageAsync` mendukung penyematan perintah OpenSpec ke dalam prompt

### 2. Pencacahan Jenis Penyedia
...
Enum ini memberikan representasi tipe-aman untuk semua penyedia yang didukung oleh sistem.

### 3. Penerapan Pola Pabrik

**`AIProviderFactory`** bertanggung jawab untuk membuat dan mengelola instance penyedia:
...
Keuntungan dari pola pabrik:
- **Caching instans**: Hindari membuat jenis penyedia yang sama berulang kali
- **Injeksi ketergantungan**: Membuat instance melalui `IServiceProvider`, dengan dukungan injeksi ketergantungan
- **Berbasis konfigurasi**: Membaca pengaturan penyedia dari file konfigurasi
- **Penanganan pengecualian**: Pengembalian `null` ketika pembuatan gagal, sehingga memudahkan lapisan atas untuk menangani kesalahan

### 4. Pemilih Cerdas

**`AIProviderSelector`** menerapkan strategi pemilihan penyedia:
...
Strategi pemilih:
- **Pemetaan skenario terlebih dahulu**: Pertama, periksa apakah skenario bisnis memiliki pemetaan penyedia tertentu
- **Pengembalian ke penyedia default**: Gunakan penyedia default jika pemetaan skenario gagal
- **Rantai cadangan sebagai perlindungan terakhir**: Coba penyedia di rantai cadangan satu per satu
- **Caching ketersediaan**: Pemeriksaan ketersediaan penyedia cache untuk mengurangi panggilan Ping

### 5. Implementasi Penyedia CLI Kode Claude
...
Karakteristik penyedia CLI Claude Code:
- **Integrasi pengelola streaming**: Gunakan `IClaudeStreamManager` untuk berkomunikasi dengan Claude CLI
- **`CessionId` isolasi sesi**: Gunakan `CessionId` sebagai pengidentifikasi sesi unik, berbeda dari sistem `sessionId`
- **Konfigurasi direktori kerja**: Mendukung konfigurasi direktori kerja, mode izin, dan banyak lagi
- **Dukungan alat**: Mendukung pengaturan izin alat seperti `AllowedTools` dan `DisallowedTools`

### 6. Implementasi Penyedia Codex CLI
...
Karakteristik penyedia Codex CLI:
- **Penanganan aliran peristiwa JSON**: Mengurai aliran peristiwa Codex JSON (`TurnStarted`, `ItemStarted`, `TurnCompleted`, dan seterusnya)
- **Pengikatan thread sesi**: Mempertahankan pengikatan antara sesi dan thread dengan database SQLite
- **Penggunaan kembali thread**: Mendukung melanjutkan thread yang ada untuk menjaga kelangsungan sesi
- **Pelacakan panggilan alat**: Melacak status panggilan alat yang aktif dan menangani siklus hidup alat dengan benar

### 7. Mekanisme Pengikatan Benang Sesi

Codex CLI menggunakan database SQLite untuk mempertahankan pengikatan antara sesi dan thread:
...
Keuntungan dari pengikatan thread sesi:
- **Pemulihan sesi**: Sesi sebelumnya dapat dipulihkan setelah sistem dimulai ulang
- **Penggunaan kembali thread**: Sesi yang sama dapat menggunakan kembali thread Codex yang sudah ada
- **Pembersihan otomatis**: Binding yang lebih lama dari 30 hari dibersihkan secara otomatis

### 8. Manajemen CLI Sisi Desktop

`hagicode-desktop` mengelola seleksi CLI melalui `AgentCliManager`:
...
Contoh pengendali IPC sisi desktop:
...
### 9. Sistem Penyedia Model Internal Codex

Codex sendiri juga mendukung beberapa penyedia model melalui `ModelProviderInfo` konfigurasi:
...
Dukungan penyedia model Codex meliputi:
- **Penyedia bawaan**: OpenAI, Ollama, dan LM Studio
- **Penyedia khusus**: Pengguna dapat menambahkan penyedia khusus ke dalamnya `config.toml`
- **Strategi percobaan ulang**: Jumlah percobaan ulang yang dapat dikonfigurasi untuk permintaan dan streaming
- **Dukungan WebSocket**: Beberapa penyedia mendukung transportasi WebSocket

## Praktek

### Contoh Konfigurasi

Konfigurasikan beberapa penyedia di **`appsettings.json`**:
...
### Contoh Penggunaan - Layanan Backend
...
### Contoh Penggunaan - Respons Streaming
...
### Contoh Penggunaan - Perintah OpenSpec
...
### 1. Pemeriksaan Kesehatan Penyedia

Sebelum berpindah penyedia, disarankan untuk menelepon `PingAsync` pertama untuk memastikan penyedia target tersedia:
...
Gunakan `CessionId` (Claude) atau `ThreadId` (Codex) untuk memastikan isolasi sesi:

- Kode Claude CLI: gunakan `CessionId` sebagai pengidentifikasi sesi unik
- Codex CLI: gunakan `ThreadId` sebagai pengidentifikasi sesi
...
Mekanisme fallback harus kuat ketika penyedia tidak tersedia, memastikan bahwa setidaknya satu penyedia tetap dapat digunakan:
...
Validasi pengaturan untuk semua penyedia yang dikonfigurasi saat startup untuk menghindari kesalahan runtime:
...
### 5. Manajemen Tembolok

Instans penyedia di-cache, jadi perhatikan manajemen siklus hidup dan penggunaan memori:
...
Pemilihan, peralihan, dan eksekusi penyedia log secara detail untuk mempermudah proses debug:
...
Menggunakan koleksi bersamaan seperti `ConcurrentDictionary` memastikan keamanan benang:
...
Ketika skema database pengikatan thread sesi berubah, migrasi data harus dipertimbangkan:
...
HagiCode menggabungkan pola penyedia, pola pabrik, dan pola pemilih untuk menerapkan arsitektur penyedia multi-AI yang fleksibel dan dapat diperluas:

- **Abstraksi antarmuka terpadu**: The `IAIProvider` antarmuka menyembunyikan perbedaan antara CLI
- **Pembuatan instance dinamis**: `AIProviderFactory` mendukung pembuatan instans penyedia waktu proses
- **Strategi seleksi cerdas**: `AIProviderSelector` mengimplementasikan pemilihan penyedia berdasarkan skenario
- **Persistensi status sesi**: Pengikatan basis data memastikan kontinuitas sesi
- **Integrasi desktop**: `AgentCliManager` mendukung pemilihan dan konfigurasi pengguna

Kelebihan arsitektur ini adalah:
1. **Ekstensibilitas**: Menambahkan penyedia AI baru hanya memerlukan penerapan `IAIProvider` antarmuka
2. **Testabilitas**: Penyedia dapat diuji dan dicontoh secara independen
3. **Kemampuan pemeliharaan**: Setiap implementasi penyedia terisolasi dan memiliki tanggung jawab tunggal
4. **Keramahan pengguna**: Mendukung pemilihan otomatis berbasis skenario dan peralihan manual

Dengan desain ini, HagiCode berhasil memungkinkan peralihan dan interoperabilitas yang mulus antara Claude Code CLI dan Codex CLI, memberikan pengembang pengalaman asisten pengkodean AI yang fleksibel dan kuat.
...
- Dokumentasi OpenAI Codex: [platform.openai.com](https://platform.openai.com)
- Repositori resmi Codex SDK: [github.com/openai/codex](https://github.com/openai/codex)
- Dukungan CLI multi-platform HagiCode: [https://docs.hagicode.com/blog/hagicode-ai-cli-multi-platform-support/](https://docs.hagicode.com/blog/hagicode-ai-cli-multi-platform-support/)
...
- Tautan artikel: [https://docs.hagicode.com/blog/2026-03-09-hagicode-multi-ai-provider-switching-interop/](https://docs.hagicode.com/blog/2026-03-09-hagicode-multi-ai-provider-switching-interop/)
- Pemberitahuan hak cipta: Kecuali dinyatakan lain, semua postingan blog di situs ini dilisensikan di bawah BY-NC-SA. Harap beri kredit pada sumbernya saat mencetak ulang!
