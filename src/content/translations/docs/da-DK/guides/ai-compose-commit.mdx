...
description: Brug AI til intelligent at generere Git-commit-beskeder, hvilket forbedrer commit-kvalitet og konsistens.
---

## Oversigt

AI Compose Commit læser de aktuelle kodeændringer og genererer commit-meddelelser i konventionelt Commit-format. Pointen er enkel: Brug mindre tid på at skrive commits, mens historien holdes klarere.

### Hvad er AI Compose Commit?

AI Compose Commit analyserer dine ændringer og skriver standardiserede commit-beskeder til dem. Det bytter manuel commit-skrivning for en mere konsistent historie.

Brug det når:
- Store kodeændringer, der kræver flere commits
- Multidepotprojekter med koordinerede ændringer
- Hold, der kræver standardiserede commit-formater
- Udviklere, der ønsker at forbedre forpligtelseskvaliteten

### Vigtigste fordele

| Feature | Traditionelle manuelle forpligtelser | AI Compose Commit |
|----------|----------------------|------------------|
| **Forpligtelse til kvalitet** | Afhænger af individuelle erfaringer | Baseret på bedste praksis, ensartet kvalitet |
| **Forpligtelseshastighed** | Kræver omtanke og organisering | Et-klik, øjeblikkelig generering |
| **Konsistens** | Kan variere mellem commits | Følger automatisk standarder |
| **Multi-repo support** | Manuel koordinering påkrævet | Automatisk genkendelse og gruppering af repo |
| **Tidsbesparelse** | Kan tage betydelig tid | Sparer 10-60+ sekunder pr. commit |

## Hurtig start

### Aktiver funktion

I HagiCode desktop-applikationen skal du navigere til det projekt, hvor du vil bruge AI Compose Commit, og aktivere denne funktion.

**Vigtigt**: Sørg for, at dit projekt er blevet tilføjet til HagiCode først.

### Trigger funktion

Find knappen "AI Combined Commit" i Git operations sektionen.

![AI Combined Commit-knap](../../img/screenshots/ai-compose-commit/trigger-ai-compose-commit-button/original.png)

Denne knap er typisk placeret fremtrædende i Git operations sektionen, i nærheden af andre versionskontrolhandlinger.

**Vigtig bemærkning**: Hvis du klikker på denne knap, udløses automatisk commit-meddelelsesgenerering. Hvis du har brug for at rulle tilbage eller ændre commits, skal du bruge standard Git-kommandoer.

### Første brugsrisikoprompt

Når du bruger AI Compose Commit for første gang eller med ikke-godkendte ændringer, viser systemet en bekræftelsesdialog.

![Anden bekræftelsesdialog](../../img/screenshots/ai-compose-commit/second-confirmation-dialog/original.png)

Dialogen indeholder muligheder, herunder:
- Bekræft og start behandlingen
- Forstå risici, vis ikke længere denne prompt
- At markere denne indstilling gør efterfølgende brug mere bekvem

Anbefal omhyggeligt at læse risikoprompten under første brug for at forstå implikationerne af automatiske forpligtelser.

### Se resultater

Når AI er færdig med behandlingen, vil du se en succesmeddelelse.

Commit-beskeder er blevet genereret og er klar. Du kan gennemgå dem, før du afslutter, hvis det er nødvendigt.

## Funktionsdetaljer

### AI analyse logik

AI Compose Commit analyserer dine kodeændringer gennem følgende proces:

#### 1. Læs Workspace Changes

Systemet samler alle ændrede, nyligt tilføjede og slettede filer i det aktuelle arbejdsområde.

#### 2. Analyser filindhold

AI analyserer dybt indholdet af ændrede filer for at forstå:
- Funktionelle ændringer gennemført
- Fejlrettelser anvendt
- Refaktoreringsmønstre brugt
- Afhængigheder tilføjet eller fjernet

#### 3. Identificer forandringsforeninger

AI identificerer intelligent, hvilke ændringer der skal grupperes sammen:
- Frontend og backend ændringer for den samme funktion
- Relaterede ændringer af konfigurationsfilen
- Test opdateringer sammen med implementeringsændringer

#### 4. Generer forpligtelsesstruktur

AI genererer struktureret commit-information efter konventionelle commit-specifikationer:
...
```
feat(auth): tilføje brugerlogin funktionalitet
Implementeret OAuth2-godkendelse med Google- og GitHub-udbydere.

Login-komponenten understøtter nu:
- Google OAuth-integration
- GitHub OAuth-integration
- Sessionsstyring med JWT-tokens
...
```text
Medforfattet af: HagiCode <noreply@hagicode.com>
```
...
```text
Medforfattet af: Platform Bot <platform-bot@example.com>
```
