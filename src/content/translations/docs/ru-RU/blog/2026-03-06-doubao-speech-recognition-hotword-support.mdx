...
description: "В этой статье подробно описывается реализация поддержки горячих слов для распознавания речи Doubao в проекте HagiCode. Используя как пользовательские горячие слова, так и"
---

## Руководство по реализации поддержки горячих слов для распознавания речи Doubao
...
Технология распознавания речи развивается уже много лет, однако одна проблема неизменно беспокоит разработчиков. Универсальные модели распознавания речи способны охватить повседневную речь, но часто не справляются с профессиональной терминологией, названиями продуктов и личными именами. Подумайте: голосовому помощнику в медицинской области необходимо точно распознавать такие термины, как «гипертония», «диабет» и «ишемическая болезнь сердца»; юридической системе — точно фиксировать такие термины, как «предмет иска», «защита» и «бремя доказывания». В этих сценариях универсальная модель старается изо всех сил, но этого часто оказывается недостаточно.
...
The solution shared in this article comes from our practical experience in the [HagiCode](https://www.hagicode.com) project. HagiCode is an open-source AI coding assistant project with a modern technology stack, designed to provide developers with an intelligent programming assistance experience. As a complex multilingual, multi-platform project, HagiCode needs to handle speech recognition scenarios involving many technical terms, which in turn drove our research into and implementation of the hotword feature.
