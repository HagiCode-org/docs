...
description: "Если говорить прямо, эта статья также во многом наша гордость: она описывает полный процесс портирования официального TypeScript Codex SDK на C#. Называть это \"портированием\""
---

## Из TypeScript в C#: практика кросс-языкового портирования Codex SDK

> Если говорить прямо, эта статья также во многом наша гордость: она описывает полный процесс портирования официального TypeScript Codex SDK на C#. Называть это «портированием» — значит делать это слишком простым — это была скорее долгая aventюра, потому что эти два языка имеют очень разные характеры, и нам пришлось найти способ заставить их сотрудничать.

## Предпосылки

Codex — это инструмент AI Agent CLI, выпущенный OpenAI, и он по-настоящему мощный. Официальная команда предоставляет TypeScript SDK в пакете `@openai/codex`. Он взаимодействует с Codex CLI, вызывая команду `codex exec --experimental-json` и парся поток событий JSONL.

Проблема заключается в том, что в проекте HagiCode нам необходимо использовать его в чистой среде .NET — конкретно в серверных службах и десктопных приложениях на C#. Мы не могли обоснованно внедрить среду выполнения Node.js в проект .NET только для вызова CLI-инструмента. Это было бы слишком громоздко.

Таким образом, у нас оставалось два варианта: поддерживать сложный слой Node.js-моста или создать нативный C# SDK самостоятельно.

Мы выбрали последний вариант.

## О HagiCode

This article also comes directly from our hands-on experience in the [HagiCode](https://www.hagicode.com) project. HagiCode is an open-source AI coding assistant project. In plain terms, it means maintaining multiple components at once: a VSCode extension on the frontend, AI services on the backend, and a cross-platform desktop client. That multi-language, multi-platform complexity is exactly why we needed a native C# SDK - we really did not want to run Node.js inside a .NET project.

If you find this article helpful, feel free to give us a star on GitHub: [github.com/HagiCode-org/site](https://github.com/HagiCode-org/site). You can also visit the official website to learn more: [hagicode.com](https://www.hagicode.com). It is always encouraging when an open-source project receives support.
...
- Быстрая установка для десктопного клиента: [hagicode.com/desktop/](https://www.hagicode.com/desktop/)
