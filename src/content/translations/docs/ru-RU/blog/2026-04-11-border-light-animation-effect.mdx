...
We ran into exactly this requirement while building [HagiCode](https://www.hagicode.com). Users needed to see at a glance which sessions were running and which proposals were currently being processed. We tried several different approaches. Some paths were smoother, some were a bit more winding. In the end, we settled on a fairly mature implementation strategy.

## О HagiCode

The approach shared in this article comes from our hands-on experience in the [HagiCode](https://www.hagicode.com) project. HagiCode is an AI-driven coding assistant project, and the interface makes extensive use of border light animations to indicate different runtime states. Examples include the running state of items in the session list, status transitions in the proposal flow diagram, and intensity indicators for throughput.

These effects are not especially complicated in principle, but we definitely ran into plenty of pitfalls while implementing them. If you want to see the real thing, you can visit our [GitHub repository](https://github.com/HagiCode-org/site) or head to the [official website](https://www.hagicode.com). In the end, what matters most is what actually works.
