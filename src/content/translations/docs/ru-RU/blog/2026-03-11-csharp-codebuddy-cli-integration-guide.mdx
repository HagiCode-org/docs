...
:::note[Time context]
В этой статье обсуждается дизайн интеграции CodeBuddy и структура провайдеров на момент публикации. Для текущего постоянно поддерживаемого объёма обратитесь к [Установке AI Agent CLI](/en/related-software-installation/ai-agent-cli/) и [Руководству по установке](/en/installation); `IFlowCli` в настоящее время сохраняется только в качестве исторического значения совместимости и не входит в текущий активно поддерживаемый объём.
:::
...
Подход, описанный в этой статье, основан на нашем практическом опыте в проекте [HagiCode](https://www.hagicode.com). HagiCode is an open-source AI coding assistant project that supports multiple AI Providers and cross-platform operation. To satisfy different user preferences, we need to switch flexibly among different AI coding assistants, which is exactly why we built the CodeBuddy integration described here.
...
### 1. Восстановление контракта провайдера
...
### 2. Интеграция фабрики провайдеров
...
### 3. Полная реализация провайдера
...
- Открытое бета-тестирование началось, приглашаем к установке и тестированию
