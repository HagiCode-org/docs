...
description: Az AI segítségével intelligensen generálhat Git commit üzeneteket, javítva ezzel a véglegesítés minőségét és konzisztenciáját.
---

## Áttekintés

Az AI Compose Commit beolvassa az aktuális kódváltozásokat, és véglegesítési üzeneteket generál hagyományos véglegesítési formátumban. A lényeg egyszerű: kevesebb időt töltsön kötelezettségvállalások megírásával, miközben a történelem tisztább marad.

### Mi az AI Compose Commit?

Az AI Compose Commit elemzi a változtatásokat, és szabványos véglegesítési üzeneteket ír nekik. A kézi commit írást a következetesebb történetért cseréli.

Akkor használja, ha:
- Nagy kódmódosítások, amelyek több véglegesítést igényelnek
- Több adattárat tartalmazó projektek összehangolt változtatásokkal
- Csapatok, amelyek szabványos kötelezettségvállalási formátumokat igényelnek
- Fejlesztők, akik szeretnének javítani a kötelezettségvállalás minőségén

### Fő előnyei

| Funkció | Hagyományos kézi kötelezettségvállalások | AI Compose Commit |
|----------|----------------------|------------------|
| **A minőség iránti elkötelezettség** | Egyéni tapasztalattól függ | A legjobb gyakorlatok alapján, állandó minőség |
| **Sebesség vállalása** | Gondolkodást és szervezést igényel | Egy kattintással, azonnali generálás |
| **Konzisztencia** | Változhat a kötelezettségvállalások között | Automatikusan követi a szabványokat |
| **Multi-repo támogatás** | Kézi koordináció szükséges | Automatikus repo felismerés és csoportosítás |
| **Időmegtakarítás** | Jelentős időt vehet igénybe | 10-60+ másodpercet takarít meg véglegesítésenként |

## Gyors kezdés

### Funkció engedélyezése

A HagiCode asztali alkalmazásban navigáljon ahhoz a projekthez, ahol használni szeretné az AI Compose Commit funkciót, és engedélyezze ezt a funkciót.

**Fontos**: Először győződjön meg róla, hogy projektjét hozzáadta a HagiCode-hoz.

### Trigger funkció

Keresse meg az „AI Combined Commit” gombot a Git-műveletek részben.

![AI kombinált véglegesítés gomb](../../img/screenshots/ai-compose-commit/trigger-ai-compose-commit-button/original.png)

Ez a gomb jellemzően jól láthatóan a Git-műveletek szakaszban található, más verzióvezérlő műveletek közelében.

**Fontos megjegyzés**: Erre a gombra kattintva automatikus véglegesítési üzenet generál. Ha vissza kell állítania vagy módosítania kell a véglegesítéseket, használja a szabványos Git parancsokat.

### Első használat kockázati figyelmeztetés

Az AI Compose Commit első használatakor vagy jóváhagyatlan változtatások esetén a rendszer egy megerősítő párbeszédpanelt jelenít meg.

![Második megerősítési párbeszédpanel](../../img/screenshots/ai-compose-commit/second-confirmation-dialog/original.png)

A párbeszédpanel a következőket tartalmazza:
- Erősítse meg és indítsa el a feldolgozást
- Értse meg a kockázatokat, ne jelenítse meg többé ezt a felszólítást
- Ennek az opciónak a bejelölése kényelmesebbé teszi a későbbi használatot

Javasoljuk, hogy az első használat során figyelmesen olvassa el a kockázati figyelmeztetést, hogy megértse az automatikus véglegesítés következményeit.

### Eredmények megtekintése

Miután a mesterséges intelligencia befejezte a feldolgozást, sikeres értesítést fog látni.

A véglegesítési üzenetek létrejöttek, és készen állnak. Ha szükséges, a véglegesítés előtt áttekintheti őket.

## Funkció részletei

### AI elemzési logika

Az AI Compose Commit a következő folyamaton keresztül elemzi a kódváltozásokat:

#### 1. Olvassa el a Munkaterület módosításai című részt

A rendszer összegyűjti az összes módosított, újonnan hozzáadott és törölt fájlt az aktuális munkaterületen.

#### 2. Elemezze a fájl tartalmát

Az AI mélyen elemzi a megváltozott fájlok tartalmát, hogy megértse:
- Funkcionális változtatások végrehajtva
- Hibajavítások alkalmazva
- Használt refaktorálási minták
- Függőségek hozzáadva vagy eltávolítva

#### 3. Azonosítsa a változási asszociációkat

Az AI intelligensen azonosítja, hogy mely változtatásokat érdemes csoportosítani:
- Változások az előtérben és a háttérben ugyanannál a szolgáltatásnál
- Kapcsolódó konfigurációs fájl módosítások
- Tesztelje a frissítéseket a megvalósítási változások mellett

#### 4. Kötelezettségi struktúra létrehozása

A mesterséges intelligencia strukturált véglegesítési információkat generál a hagyományos véglegesítési specifikáció szerint:
...
```
feat(auth): felhasználói bejelentkezési funkció hozzáadása
Megvalósított OAuth2 hitelesítés a Google és a GitHub szolgáltatókkal.

A bejelentkezési komponens mostantól támogatja:
- Google OAuth integráció
- GitHub OAuth integráció
- Munkamenet menedzsment JWT tokenekkel

Társszerző: Claude Opus 4.6 <noreply@anthropic.com>
```
...
```text
Társszerző: HagiCode <noreply@hagicode.com>
```
...
git config --global coAuthoredBy.e-mail: "noreply@hagicode.com"
```

With the local override above, the trailer becomes:

```text
Társszerző: Platform Bot <platform-bot@example.com>
```
