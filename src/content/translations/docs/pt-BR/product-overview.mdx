...
Depois de entender o que o HagiCode está tentando ser, a próxima questão prática geralmente é simples: com qual edição você deve começar agora e qual é o canal público atual do Windows?

Na tabela abaixo, `✅` significa suportado e `❌` significa não suportado.

| Artigo | Área de trabalho | Recipiente | Microsoft Store |
| --- | --- | --- | --- |
| Ponto de entrada | [Instalação na área de trabalho](/pt-BR/installation/desktop) | [Implantação de contêiner](/pt-BR/installation/docker-compose) | [Instalação pela Microsoft Store](/pt-BR/installation/windows-store) |
| Preços | Grátis | Grátis | Grátis |
...
| Integração com GitHub | ✅ | ✅ | ✅ |
| Gerenciamento Git | ✅ | ✅ | ✅ |
| Máximo de propostas simultâneas | 3 | 3 | 3 |
| Instalação pela loja do sistema | ❌ | ❌ | ✅ |

**Status do canal Windows.** A entrada principal do aplicativo na Steam já foi retirada. A Microsoft Store agora é o canal público de distribuição para Windows. Para o contexto, consulte [Por que o HagiCode não está disponível no Steam](/pt-BR/faq/steam-distribution-status).

**Alinhamento de capacidades.** A edição atual da Microsoft Store se alinha à base gratuita do Desktop nas capacidades centrais do fluxo de trabalho, incluindo propostas, Vault, Skills, OmniRoute, integração com GitHub e conquistas locais.

**Recomendação de instalação.** Se você prefere um caminho de instalação e atualização gerenciado pelo sistema, escolha a Microsoft Store. Se precisa controlar manualmente a origem do pacote ou usar um caminho fora da loja, use [Desktop Installation](/pt-BR/installation/desktop).

### Canal atual do Windows

<CardGrid>
  <LinkCard
  title="Instalação pela Microsoft Store"
  href="/pt-BR/installation/windows-store"
  description="O caminho público atual para Windows se você quer que a instalação e a entrega do pacote sejam gerenciadas pela loja do sistema."
/>
  <LinkCard
  title="Status da distribuição na Steam"
  href="/pt-BR/faq/steam-distribution-status"
  description="Explica por que a entrada principal na Steam foi retirada e por que o canal do Windows mudou para a Microsoft Store."
/>
</CardGrid>
