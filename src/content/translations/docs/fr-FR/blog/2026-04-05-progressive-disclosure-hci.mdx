...
description: "In AI product design, the quality of user input often determines the quality of the output. Cet article partage a \"progressive disclosure\" interaction approach"
---

## Progressive Disclosure: Improving Human-Computer Interaction in AI Products with the "Less Is More" Philosophy

> In AI product design, the quality of user input often determines the quality of the output. Cet article partage a "progressive disclosure" interaction approach we practiced in the HagiCode projet. À travers step-by-step guidance, intelligent completion, and instant feedback, it turns users' brief and vague inputs into structured technical proposals, significantly improving human-computer interaction efficiency.
...
## Quoi Is Progressive Disclosure
...
Quand a user enters a short description, we do not send it directly to the AI for interpretation. Instead, we first trigger a "description optimization" flow. The core of this flow is "structured output": converting the user's free text into a standard format. It is like stringing loose pearls into a necklace so everything becomes easier to understand.

The optimized description must include the following standard sections:

- **Contexte**: the problem background and context
...
En même temps, we automatically generate a Markdown table showing information such as the target repository, paths, and edit permissions, making subsequent AI operations easier. A clear directory always makes things easier to find.
...
Chaque prompt consists of two parts:
- **Metadata file ( IB6 )**: defines information such as the prompt scenario, version, and parameters
- **Template file ( IB7 )**: the actual prompt content, written with Handlebars syntax
...
For complex tasks, such as first-time installation and configuration, we use a multi-step wizard design. Chaque step requests only the necessary information and provides clear progress indicators. Large tasks become much more manageable when handled one step at a time.
...
Chaque step has its own validation logic, and completed steps receive clear visual markers. Canceling opens a confirmation dialog to prevent users from losing progress through accidental actions.
...
If this article helped you, feel free to give us a Star on GitHub and follow the continued development of the HagiCode projet. Public beta has already started, and you can experience the full feature set right now by installing it:
...
- Quick installation for the Desktop app: [hagicode.com/desktop/](https://www.hagicode.com/desktop/)

## Copyright Notice

Merci de votre lecture. If you found this article useful, feel free to like, save, and share it.
...
- Licence : Sauf mention contraire, tous les articles de ce blog sont sous licence BY-NC-SA. Please cite the source when reprinting.
