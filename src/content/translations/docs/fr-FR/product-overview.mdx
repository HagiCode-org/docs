...
Une fois que vous avez compris ce que HagiCode essaie d'être, la question pratique suivante est généralement simple : par quelle édition devriez-vous commencer maintenant et quel est le canal public actuel pour Windows ?

Dans le tableau ci-dessous, `✅` moyens soutenus et `❌` signifie non pris en charge.

| Article | Bureau | Conteneur | Microsoft Store |
| --- | --- | --- | --- |
| Point d'entrée | [Installation de bureau](/fr-FR/installation/desktop) | [Déploiement de conteneurs](/fr-FR/installation/docker-compose) | [Installation depuis Microsoft Store](/fr-FR/installation/windows-store) |
...
| Intégration GitHub | ✅ | ✅ | ✅ |
| Gestion de Git | ✅ | ✅ | ✅ |
| Nombre maximal de propositions simultanées | 3 | 3 | 3 |
| Installation via le store système | ❌ | ❌ | ✅ |

**État du canal Windows.** L’entrée principale de l’application sur Steam a déjà été retirée. Microsoft Store est désormais le canal public de distribution Windows. Pour le contexte, consultez [Pourquoi HagiCode n’est pas disponible sur Steam](/fr-FR/faq/steam-distribution-status).

**Alignement des capacités.** L’édition actuelle Microsoft Store s’aligne sur la base gratuite Desktop pour les capacités principales du flux de travail, y compris les propositions, Vault, Skills, OmniRoute, l’intégration GitHub et les réalisations locales.

**Recommandation d’installation.** Si vous préférez un chemin d’installation et de mise à jour géré par le système, choisissez Microsoft Store. Si vous devez contrôler manuellement la source du package ou utiliser une voie hors Store, utilisez plutôt [Desktop Installation](/fr-FR/installation/desktop).

### Canal Windows actuel

<CardGrid>
  <LinkCard
  title="Installation depuis Microsoft Store"
  href="/fr-FR/installation/windows-store"
  description="Le chemin public actuel pour Windows si vous voulez que l’installation et la livraison du package soient gérées par le store système."
/>
  <LinkCard
  title="État de la distribution Steam"
  href="/fr-FR/faq/steam-distribution-status"
  description="Explique pourquoi l’entrée principale sur Steam a été retirée et pourquoi le canal Windows est passé à Microsoft Store."
/>
</CardGrid>
