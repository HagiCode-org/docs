...
This article records the multi-CLI expansion progress as it stood at the time of release. For the current always-supported scope, refer to [AI Agent CLI Installation](/en-US/related-software-installation/ai-agent-cli/) and the [Installation Guide](/en-US/installation); the `IFlowCli` mentioned here only represents a historical configuration at that time and is not part of the current active support scope.
:::
...
The approach shared in this article comes from our practical experience in the [HagiCode](https://www.hagicode.com) project. HagiCode is an open-source AI-assisted coding platform designed to provide developers with a flexible and powerful AI programming assistant through a multi-model, multi-CLI architecture. Project repository: [github.com/HagiCode-org/site](https://github.com/HagiCode-org/site)
