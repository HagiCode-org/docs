...
This article records the multi-Agent configuration practice **as released on 2026-03-17**. For the current long-term supported scope, please refer to [AI Agent CLI Installation](/en-US/related-software-installation/ai-agent-cli/) and the [Installation Guide](/en-US/installation); `IFlowCli` is now retained only as a legacy-compatible value and is no longer part of the actively supported scope.
:::
