---
title: FAQ
description: Sehen Sie sich allgemeine HagiCode-Laufzeithinweise, Startvorbehalte, Erklärungen zur Laufzeitumgebung und zukünftige Einträge zur Fehlerbehebung an einem Ort an.
sidebar:
  order: 1
---

import { CardGrid, LinkCard } from '@astrojs/starlight/components';

Auf dieser Seite werden häufig gestellte Fragen, Laufzeitanleitungen und Einstiegspunkte zur Fehlerbehebung für HagiCode gesammelt.

Aktuelle FAQ-Einträge behandeln die gebündelte Node-Toolchain, die von Desktop-verwalteten Flows verwendet wird.

## Aktuelle FAQ-Einträge

<CardGrid>
  <LinkCard
  title="Desktop Node Environment"
  href="/de-DE/faq/desktop-node-environment"
  description="Understand why Desktop bundles Node.js, how it prefers bundled Node paths at runtime, and why it does not replace your system Node."
/>
  <LinkCard
    title="Warum HagiCode nicht auf Steam verfügbar ist"
    href="/de-DE/faq/steam-distribution-status"
    description="Erfahren Sie, warum die Steam-Prüfung abgelehnt wurde, wie das mit dem aktuellen Produktmodell von HagiCode zusammenhängt und was wir über künftige Vertriebskanäle sagen können."
  />
</CardGrid>

## Was dieser Abschnitt behandelt

Der FAQ-Bereich ist der Ort für:

- Einschränkungen beim Start und bei der Laufzeit
- Kompatibilitätshinweise zu Plattformen, Vertriebskanälen, Verpackungen oder gebündelten Laufzeiten
- Fehlerbehebungsthemen, die eine stabile eigenständige Erklärungsseite verdienen

Wenn Sie bestätigen müssen, welcher Node von Desktop verwendet wird oder warum die Diagnose `portable-fixed/toolchain` erwähnt, öffnen Sie [Desktop-Knotenumgebung](/de-DE/faq/desktop-node-environment).

Wenn Sie wissen möchten, warum HagiCode nicht auf Steam ist, öffnen Sie [Warum HagiCode nicht auf Steam verfügbar ist](/de-DE/faq/steam-distribution-status).
